...
async fn test_conditional_get_etag() -> anyhow::Result<()> {
    ...
    let cc = CreateBucketConfiguration::builder()
        .location_constraint(loc)
        .build();
    let _ = client
        .create_bucket()
        .create_bucket_configuration(cc)
        .bucket(bucket)
        .send()
        .await
    ...
    let _out = client
        ...
        .send()
        .await
        .context("get with if-match match")?;
    ...
    if let Err(e) = res {
        ...
        assert!(
            code.contains("PreconditionFailed") || code.contains("412"),
    ...
    let out = client
        ...
        .bucket(bucket)
        .key(key)
        .if_none_match("nonmatchingetag123")
        .send()
        .await
        .context("get with if-none-match non-matching")?;
...
async fn test_conditional_get_modified_since() -> anyhow::Result<()> {
    ...
    let out = client
        ...
        .send()
        .await
        .context("get with if-modified-since past")?;
    ...
    let out = client
        ...
        .send()
        .await
        .context("get with if-unmodified-since future")?;
    assert!(out.e_tag().is_some());
